...
class PreprocessPlusAugment(nn.Module):
    ...
    def forward(self, datas, output_dir, is_stereo=False, augment_small=False, n_continuous=4):
        ...
        img12, img12_depth, back_flow12 = img1_all_fw[0:3], img1_all_fw[3:4], img1_all_fw[4:6]
        img12 = utils.inpaint(img12, valid, collision)
        img12_depth = utils.fix_warped_depth(img12_depth)

        flow02 = self.cf(flow01, back_flow01, flow12, img1_depth)
        # back_flow02 = self.bf(flow02, img0_depth)
        img0_all = torch.cat((img0, img0_depth, flow02 * -1), axis=0) 
        img0_all_fw, valid, collision = self.fw(img0_all, flow02, img0_depth)
        img0_all_fw = img0_all_fw.squeeze(0) 
        img02, img02_depth, back_flow02 = img0_all_fw[0:3], img0_all_fw[3:4], img0_all_fw[4:6]
        img02 = utils.inpaint(img02, valid, collision)
        img02_depth = utils.fix_warped_depth(img02_depth)

        img2 = img02
        img2_depth = img02_depth
        ...
        # warped_img2, _, _ = self.fw(img1, flow12, img1_depth)
        ...
        flow0i = flow02
        back_flow0i = back_flow02
        for i in range(3, 2 + n_continuous):
            flowii1, _ = Convert.depth_to_random_flow(imgi_depth, self.device, T1=T1)
            back_flowii1 = self.bf(flowii1, imgi_depth)
            flow0i1 = self.cf(flow0i, back_flow0i, flowii1, imgi_depth)
            img0_all = torch.cat((img0, img0_depth, flow0i1 * -1), axis=0) 
            img0_all_fw, valid, collision = self.fw(img0_all, flow0i1, img0_depth)
            img0_all_fw = img0_all_fw.squeeze(0) 
            imgi1, imgi1_depth, back_flow0i1 = img0_all_fw[0:3], img0_all_fw[3:4], img0_all_fw[4:6]
            ...
            data = torch.cat((imgi1, imgi1_depth, flowii1, back_flowii1, flow0i1, back_flow0i1), axis=0)
                            #   0:3,         3:4,     4:6,          6:8,    8:10,        10:12
                            # 12:15,       15:16,   16:18,        18:20,   20:22,        22:24
                            # 24:27,       27:28,   28:30,        30:32,   32:34,        34:36
            continuous_data.append(data)

            imgi = imgi1
            imgi_depth = imgi1_depth
            flow0i = flow0i1
            back_flow0i = back_flow0i1
        ...
        img0_all = torch.cat((img0, img0_depth, flow0a * -1), axis=0) 
        ...
        continuous_data = [torch.cat((imga, imga_depth, flow0a, back_flow0a, flow0a, back_flow0a), axis=0)]
                                   #   0:3,        3:4,    4:6,         6:8,   8:10,       10:12
        imgi = imga
        ...
        for i in range(3 + n_continuous, 2 + n_continuous * 2):
            ...
            data = torch.cat((imgi1, imgi1_depth, flowii1, back_flowii1, flow0i1, back_flow0i1), axis=0)
                            # 12:15,       15:16,   16:18,        18:20,   20:22,        22:24
                            # 24:27,       27:28,   28:30,        30:32,   32:34,        34:36
                            # 36:39,       39:40,   40:42,        42:44,   44:46,        46:48
            continuous_data.append(data)
        ...
        del img0, img1, img02, img12, img2
        del img0_depth, img1_depth, img02_depth, img12_depth, img2_depth
        del disp0
        ...
        del to_save_data0, to_save_data3, to_save_data4
